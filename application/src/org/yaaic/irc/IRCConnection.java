...
public class IRCConnection extends PircBot {
	 ...
	 */
	@Override
	protected void handleLine(String line) throws NickAlreadyInUseException,
			IOException {
		if (debugTraffic) {
	...
	public void onConnect() {
		...
		service.sendBroadcast(Broadcast.createServerIntent(
				Broadcast.SERVER_UPDATE, server.getId()));

		service.notifyConnected(server.getTitle());

		Message message = new Message(service.getString(
				R.string.message_connected, server.getTitle()));
		message.setColor(Message.COLOR_GREEN);
		server.getConversation(ServerInfo.DEFAULT_NAME).addMessage(message);

		Message infoMessage = new Message(
				service.getString(R.string.message_now_login));
	...
	public void onRegister() {
		...
		for (String command : server.getConnectCommands()) {
			parser.parse(command, server,
					server.getConversation(ServerInfo.DEFAULT_NAME), service);
		}

		// TODO: Detect "You are now identified for <nick>" notices from
		// NickServ and handle
		// auto joins in onNotice instead if the user has chosen to wait for
		// NickServ
		...
		Message infoMessage = new Message(
				service.getString(R.string.message_login_done));
		infoMessage.setColor(Message.COLOR_GREY);
		server.getConversation(ServerInfo.DEFAULT_NAME).addMessage(infoMessage);
	...
	protected void onAction(String sender, String login, String hostname,
		...
		if (conversation == null) {
			...
			Intent intent = Broadcast.createConversationIntent(
					Broadcast.CONVERSATION_NEW, server.getId(), queryNick);
			service.sendBroadcast(intent);
		} else {
			conversation.addMessage(message);
		...
		if (mentioned || target.equals(this.getNick())) {
			if (conversation.getStatus() != Conversation.STATUS_SELECTED
				...
				service.addNewMention(server.getId(), conversation,
	 ...
	 */
	@Override
	protected void onDeop(String target, String sourceNick, String sourceLogin,
	 ...
	 */
	@Override
	protected void onDeVoice(String target, String sourceNick,
			String sourceLogin, String sourceHostname, String recipient) {
		Message message = new Message(service.getString(
				R.string.message_devoice, sourceNick, recipient));
	...
	protected void onInvite(String targetNick, String sourceNick,
		...
		if (targetNick.equals(this.getNick())) {
			// We are invited
			Message message = new Message(service.getString(
					R.string.message_invite_you, sourceNick, target));
			...
			// Someone is invited
			Message message = new Message(service.getString(
					R.string.message_invite_someone, sourceNick, targetNick,
					target));
			server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onJoin(String target, String sender, String login,
			String hostname) {
		if (sender.equalsIgnoreCase(getNick())
			...
			Intent intent = Broadcast.createConversationIntent(
					Broadcast.CONVERSATION_NEW, server.getId(), target);
			service.sendBroadcast(intent);
		} else if (service.getSettings().showJoinPartAndQuit()) {
			Message message = new Message(service.getString(
					R.string.message_join, sender), Message.TYPE_MISC);
	 ...
	 */
	@Override
	protected void onKick(String target, String kickerNick, String kickerLogin,
		...
		if (recipientNick.equals(getNick())) {
			...
			Intent intent = Broadcast.createConversationIntent(
					Broadcast.CONVERSATION_REMOVE, server.getId(), target);
			service.sendBroadcast(intent);
		} else {
			Message message = new Message(service.getString(
					R.string.message_kick, kickerNick, recipientNick));
			message.setColor(Message.COLOR_GREEN);
			server.getConversation(target).addMessage(message);
	...
	protected void onMessage(String target, String sender, String login,
		...
		if (isMentioned(text)) {
			...
			if (conversation.getStatus() != Conversation.STATUS_SELECTED
					|| !server.getIsForeground()) {
				service.addNewMention(server.getId(), conversation, target
						+ ": <" + sender + "> " + text, service.getSettings()
						.isVibrateHighlightEnabled(), service.getSettings()
						.isSoundHighlightEnabled(), service.getSettings()
						.isLedHighlightEnabled());
			}
	 ...
	 */
	@Override
	protected void onMode(String target, String sourceNick, String sourceLogin,
	...
	protected void onNickChange(String oldNick, String login, String hostname,
		...
		if (getNick().equalsIgnoreCase(newNick)) {
			...
			Message message = new Message(service.getString(
					R.string.message_self_rename, newNick));
			message.setColor(Message.COLOR_GREEN);
			server.getConversation(ServerInfo.DEFAULT_NAME).addMessage(message);
		...
		for (String target : channels) {
			Message message = new Message(service.getString(
					R.string.message_rename, oldNick, newNick));
			message.setColor(Message.COLOR_GREEN);
			server.getConversation(target).addMessage(message);
	...
	protected void onNotice(String sourceNick, String sourceLogin,
		...
		if (service.getSettings().showNoticeInServerWindow()) {
			...
			conversation = server.getConversation(server
	 ...
	 */
	@Override
	protected void onOp(String target, String sourceNick, String sourceLogin,
			String sourceHostname, String recipient) {
		Message message = new Message(service.getString(R.string.message_op,
				sourceNick, recipient));
		message.setColor(Message.COLOR_BLUE);
	 ...
	 */
	@Override
	protected void onPart(String target, String sender, String login,
			String hostname) {
		if (sender.equals(getNick())) {
			...
			Message message = new Message(service.getString(
					R.string.message_part, sender), Message.TYPE_MISC);
	...
	protected void onPrivateMessage(String sender, String login,
			String hostname, String target, String text) {
		...
		if (conversation.getStatus() != Conversation.STATUS_SELECTED
				|| !server.getIsForeground()) {
			service.addNewMention(server.getId(), conversation, "<" + sender
					+ "> " + text, service.getSettings()
					.isVibrateHighlightEnabled(), service.getSettings()
					.isSoundHighlightEnabled(), service.getSettings()
					.isLedHighlightEnabled());
		}
	...
	protected void onQuit(String sourceNick, String sourceLogin,
		...
		if (service.getSettings().showJoinPartAndQuit()) {
			...
			for (String target : channels) {
				Message message = new Message(service.getString(
						R.string.message_quit, sourceNick, reason),
						Message.TYPE_MISC);
	 ...
	 */
	@Override
	public void onTopic(String target, String topic, String setBy, long date,
			boolean changed) {
		if (changed) {
			Message message = new Message(service.getString(
					R.string.message_topic_set, setBy, topic));
			message.setColor(Message.COLOR_YELLOW);
			server.getConversation(target).addMessage(message);
		} else {
			Message message = new Message(service.getString(
					R.string.message_topic, topic));
			message.setColor(Message.COLOR_YELLOW);
			server.getConversation(target).addMessage(message);
		}
		...
		intent = Broadcast.createConversationIntent(
				Broadcast.CONVERSATION_TOPIC, server.getId(), target);
		service.sendBroadcast(intent);
	}
	...
	protected void onUserList(String channel, User[] users) {
		// XXX: Store user list somewhere and keep it updated or just broadcast
		// some event?
	}
	 ...
	 */
	@Override
	protected void onVoice(String target, String sourceNick,
			String sourceLogin, String sourceHostname, String recipient) {
		Message message = new Message(service.getString(R.string.message_voice,
				sourceNick, recipient));
		message.setIcon(R.drawable.voice);
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onRemoveChannelKey(String target, String sourceNick,
			String sourceLogin, String sourceHostname, String key) {
		Message message = new Message(service.getString(
				R.string.message_remove_channel_key, sourceNick));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onSetChannelKey(String target, String sourceNick,
			String sourceLogin, String sourceHostname, String key) {
		Message message = new Message(service.getString(
				R.string.message_set_channel_key, sourceNick, key));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onSetSecret(String target, String sourceNick,
			String sourceLogin, String sourceHostname) {
		Message message = new Message(service.getString(
				R.string.message_set_channel_secret, sourceNick));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onRemoveSecret(String target, String sourceNick,
			String sourceLogin, String sourceHostname) {
		Message message = new Message(service.getString(
				R.string.message_set_channel_public, sourceNick));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onSetChannelLimit(String target, String sourceNick,
			String sourceLogin, String sourceHostname, int limit) {
		Message message = new Message(service.getString(
				R.string.message_set_channel_limit, sourceNick, limit));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onRemoveChannelLimit(String target, String sourceNick,
			String sourceLogin, String sourceHostname) {
		Message message = new Message(service.getString(
				R.string.message_remove_channel_limit, sourceNick));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onSetChannelBan(String target, String sourceNick,
			String sourceLogin, String sourceHostname, String hostmask) {
		Message message = new Message(service.getString(
				R.string.message_set_ban, sourceNick, hostmask));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onRemoveChannelBan(String target, String sourceNick,
			String sourceLogin, String sourceHostname, String hostmask) {
		Message message = new Message(service.getString(
				R.string.message_remove_ban, sourceNick, hostmask));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onSetTopicProtection(String target, String sourceNick,
			String sourceLogin, String sourceHostname) {
		Message message = new Message(service.getString(
				R.string.message_set_topic_protection, sourceNick));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onRemoveTopicProtection(String target, String sourceNick,
			String sourceLogin, String sourceHostname) {
		Message message = new Message(service.getString(
				R.string.message_remove_topic_protection, sourceNick));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onSetNoExternalMessages(String target, String sourceNick,
			String sourceLogin, String sourceHostname) {
		Message message = new Message(service.getString(
				R.string.message_disable_external, sourceNick));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onRemoveNoExternalMessages(String target, String sourceNick,
			String sourceLogin, String sourceHostname) {
		Message message = new Message(service.getString(
				R.string.message_enable_external, sourceNick));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onSetInviteOnly(String target, String sourceNick,
			String sourceLogin, String sourceHostname) {
		Message message = new Message(service.getString(
				R.string.message_set_invite_only, sourceNick));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onRemoveInviteOnly(String target, String sourceNick,
			String sourceLogin, String sourceHostname) {
		Message message = new Message(service.getString(
				R.string.message_remove_invite_only, sourceNick));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onSetModerated(String target, String sourceNick,
			String sourceLogin, String sourceHostname) {
		Message message = new Message(service.getString(
				R.string.message_set_moderated, sourceNick));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onRemoveModerated(String target, String sourceNick,
			String sourceLogin, String sourceHostname) {
		Message message = new Message(service.getString(
				R.string.message_remove_moderated, sourceNick));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onSetPrivate(String target, String sourceNick,
			String sourceLogin, String sourceHostname) {
		Message message = new Message(service.getString(
				R.string.message_set_channel_private, sourceNick));
		message.setColor(Message.COLOR_BLUE);
		server.getConversation(target).addMessage(message);
	 ...
	 */
	@Override
	protected void onRemovePrivate(String target, String sourceNick,
			String sourceLogin, String sourceHostname) {
		Message message = new Message(service.getString(
				R.string.message_set_channel_public, sourceNick));
		message.setColor(Message.COLOR_BLUE);
	...
	protected void onServerResponse(int code, String response) {
		...
		if (code == 401 || code == 311 || code == 312 || code == 313 || code == 317
				|| code == 318 || code == 319) {
			...
			case 401:
				if (split.length >= 1)
					whoisNick = split[1];
				response = whoisNick + " :No such nick/channel";
                break;
			case 311:
				if (split.length >= 1)
					whoisNick = split[1];
				...
				response = whoisNick + " is " + whoisUser + "@" + whoisHost
						+ " * " + whoisName;
				break;
			case 312:
				if (split.length >= 1)
				...
				if (split.length >= 2)
					whoisServer = split[2];
				if (split.length >= 3)
					whoisServerInfo = split[3];

				response = whoisNick + " is connecting from " + whoisServer
						+ " " + whoisServerInfo;
				break;
			case 313:
				if (split.length >= 1)
				...
				response = whoisNick + " is an IRC operator";
				break;
			case 317:
				if (split.length >= 1)
					whoisNick = split[1];
				...
				Integer seconds = Integer.valueOf(whoisIdle);
				Integer hours = seconds / 3600;
				seconds = seconds - hours * 3600;
				Integer minutes = seconds / 60;
				seconds = seconds - minutes * 60;

				response = whoisNick + " has been idle " + hours.toString()
						+ "h " + minutes.toString() + "m " + seconds.toString()
						+ "s";
				break;
			case 319:
				if (split.length >= 1)
			...
			case 318:
				if (split.length >= 1)
					whoisNick = split[1];
				response = whoisNick + " End of /WHOIS list.";
				break;
			}
		...
		if (code == 376 && ignoreMOTD) {
			Message motdMessage = new Message(
					service.getString(R.string.message_motd_suppressed));
			motdMessage.setColor(Message.COLOR_GREY);
			server.getConversation(ServerInfo.DEFAULT_NAME).addMessage(
					motdMessage);
			ignoreMOTD = false;
			return;
		}
	...
	public void onDisconnect() {
		...
		for (Conversation conversation : conversations) {
			Message message = new Message(
					service.getString(R.string.message_disconnected));
	 ...
	 * @param nickname
	 *            The nickname of the user (with or without prefix)
	 ...
	 * 
	 * @param text
	 *            The text to check for the nickname
	...
	/**
	 * Update the nick matching pattern, should be called when the nickname
	 * changes.
	 */
	private void updateNickMatchPattern() {
		mNickMatch = Pattern.compile(
				"(?:^|[\\s?!'�:;,.])" + Pattern.quote(getNick())
