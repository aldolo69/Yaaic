...
public class ConversationActivity extends SherlockActivity implements
	...
	private boolean tabAsMIRC = false;
	private boolean keepSoftkeyOpen = false;
	...
	private final OnKeyListener inputKeyListener = new OnKeyListener() {
		...
		public boolean onKey(View view, int keyCode, KeyEvent event) {
			...
			// use tab as search key like in MIRC
			if (keyCode == KeyEvent.KEYCODE_TAB && tabAsMIRC == true) {
				doNickCompletion(input);
				return true;
			}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tabAsMIRC = settings.isTabAsMircEnabled();
		keepSoftkeyOpen = settings.isKeepSoftkeyOpenEnabled();
		...
		if (keepSoftkeyOpen == true) {
			input.setImeOptions(input.getImeOptions() & 0xfffffff8
				...
				public void run() {
					...
					case User.ACTION_QUERY:
						...
						if (query == null) {
							...
							query.setHistorySize(binder.getService()
									.getSettings().getHistorySize());
