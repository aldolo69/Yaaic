 ...
 * state of the page.
 */
public class ConversationTitlePageIndicator extends View implements
		PageIndicator {
	...
	public int getChannelMaxSize() {
		return mChannelMaxSize;
	}

	public void setChannelMaxSize(int size) {
		mChannelMaxSize = size;
		invalidate();
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < count; i++) {
			...
			if ((bound.left > left && bound.left < right)
				...
				mPaintText.setColor(currentPage ? mColorText : mStateProvider
						.getColorAt(i));
	...
	public void setViewPager(ViewPager view) {
		...
		if (!(mPagerAdapter instanceof ConversationStateProvider)) {
			throw new IllegalStateException(
					"Adapter has to implement ConversationStateProvider.");
		}
	...
	private CharSequence getTitle(int i) {
		...
		if (mChannelMaxSize > 0 && title.length() > mChannelMaxSize) {
			title = title.subSequence(0, mChannelMaxSize) + "\u2026";
		}
