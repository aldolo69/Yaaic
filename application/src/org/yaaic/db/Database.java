...
public class Database extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TABLE " + FavoriteConstants.TABLE_NAME + " ("
				+ FavoriteConstants._ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT, "
				+ FavoriteConstants.NAME + " TEXT NOT NULL " + ");");
	}
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		...
		if (oldVersion == 1) {
			...
			db.execSQL("ALTER TABLE " + ServerConstants.TABLE_NAME + " ADD "
					+ ServerConstants.CHARSET + " TEXT AFTER "
					+ ServerConstants.USE_SSL + ";");
		...
		if (oldVersion == 4) {
			...
			db.execSQL("ALTER TABLE " + ServerConstants.TABLE_NAME + " ADD "
					+ ServerConstants.NICKSERV_PASSWORD + " TEXT AFTER "
					+ ServerConstants.CHARSET + ";");
			db.execSQL("ALTER TABLE " + ServerConstants.TABLE_NAME + " ADD "
					+ ServerConstants.SASL_USERNAME + " TEXT AFTER "
					+ ServerConstants.NICKSERV_PASSWORD + ";");
			db.execSQL("ALTER TABLE " + ServerConstants.TABLE_NAME + " ADD "
					+ ServerConstants.SASL_PASSWORD + " TEXT AFTER "
					+ ServerConstants.SASL_USERNAME + ";");
		}
		if (oldVersion < 6) {
			db.execSQL("CREATE TABLE " + FavoriteConstants.TABLE_NAME + " ("
					+ FavoriteConstants._ID
	 ...
	 *            The server to add.
	 * @param identityId
	 *            The id of the assigned identity
	 */
	public long addServer(Server server, int identityId) {
		...
		values.put(ServerConstants.NICKSERV_PASSWORD,
				authentication.getNickservPassword());
		values.put(ServerConstants.SASL_USERNAME,
				authentication.getSaslUsername());
		values.put(ServerConstants.SASL_PASSWORD,
				authentication.getSaslPassword());

		return this.getWritableDatabase().insert(ServerConstants.TABLE_NAME,
				null, values);
	}
	 ...
	 * 
	 * @param serverId
	 *            The primary key of the server to update.
	 * @param server
	 *            The server to update.
	 * @param identityId
	 *            The identity of the server record
	 */
	public void updateServer(int serverId, Server server, int identityId) {
		...
		this.getWritableDatabase().update(ServerConstants.TABLE_NAME, values,
				ServerConstants._ID + " = " + serverId, null);
	}
	...
	public void addChannel(int serverId, String name, String password) {
		...
		this.getWritableDatabase().insert(ChannelConstants.TABLE_NAME, null,
				values);
	}
	...
	public void setChannels(int serverId, ArrayList<String> channels) {
		// Remove old channels
		this.getWritableDatabase().delete(ChannelConstants.TABLE_NAME,
				ChannelConstants.SERVER + " = " + serverId, null);
	...
	public ArrayList<String> getCommandsByServerId(int serverId) {
		...
		Cursor cursor = this.getReadableDatabase().query(
				CommandConstants.TABLE_NAME, CommandConstants.ALL,
				CommandConstants.SERVER + " = " + serverId, null, null, null,
				null);

		while (cursor.moveToNext()) {
			String command = cursor.getString(cursor
					.getColumnIndex(CommandConstants.COMMAND));
	 ...
	 *            Unique id of server
	 * @param command
	 *            The command to execute after connect
	 */
	public void addCommand(int serverId, String command) {
		...
		this.getWritableDatabase().insert(CommandConstants.TABLE_NAME, null,
				values);
	}
	 ...
	 *            Unique id of server
	 * @param commands
	 *            List of commands to execute after connect
	 */
	public void setCommands(int serverId, ArrayList<String> commands) {
		// Remove old commands
		this.getWritableDatabase().delete(CommandConstants.TABLE_NAME,
				CommandConstants.SERVER + " = " + serverId, null);
	...
	public HashMap<Integer, Server> getServers() {
		...
		Cursor cursor = this.getReadableDatabase().query(
				ServerConstants.TABLE_NAME, ServerConstants.ALL, null, null,
				null, null, ServerConstants.TITLE + " ASC");
	...
	public Server getServerById(int serverId) {
		...
		Cursor cursor = this.getReadableDatabase().query(
				ServerConstants.TABLE_NAME, ServerConstants.ALL,
				ServerConstants._ID + " = " + serverId, null, null, null,
	...
	public boolean isTitleUsed(String title) {
		...
		Cursor cursor = this.getReadableDatabase().query(
				...
				ServerConstants.ALL,
				ServerConstants.TITLE + " = "
						+ DatabaseUtils.sqlEscapeString(title), null, null,
				null, null);
	...
	private Server populateServer(Cursor cursor) {
		...
		server.setTitle(cursor.getString(cursor
				.getColumnIndex((ServerConstants.TITLE))));
		server.setHost(cursor.getString(cursor
				.getColumnIndex((ServerConstants.HOST))));
		server.setPort(cursor.getInt(cursor
				.getColumnIndex((ServerConstants.PORT))));
		server.setPassword(cursor.getString(cursor
				.getColumnIndex(ServerConstants.PASSWORD)));
		server.setId(cursor.getInt(cursor.getColumnIndex((ServerConstants._ID))));
		server.setCharset(cursor.getString(cursor
				.getColumnIndex(ServerConstants.CHARSET)));

		String useSSLvalue = cursor.getString(cursor
				.getColumnIndex(ServerConstants.USE_SSL));
		...
		authentication.setNickservPassword(cursor.getString(cursor
				.getColumnIndex(ServerConstants.NICKSERV_PASSWORD)));
		authentication.setSaslUsername(cursor.getString(cursor
				.getColumnIndex(ServerConstants.SASL_USERNAME)));
		authentication.setSaslPassword(cursor.getString(cursor
				.getColumnIndex(ServerConstants.SASL_PASSWORD)));
		server.setAuthentication(authentication);

		// Load identity for server
		Identity identity = this.getIdentityById(cursor.getInt(cursor
				.getColumnIndex(ServerConstants.IDENTITY)));
	...
	public Cursor getAutoConnectServers() {
		return this.getReadableDatabase().query(ServerConstants.TABLE_NAME,
				ServerConstants.ALL, ServerConstants.AUTOCONNECT + " = 1",
				null, null, null, ServerConstants.TITLE + " ASC");
	}
	...
	public ArrayList<String> getChannelsByServerId(int serverId) {
		...
		Cursor cursor = this.getReadableDatabase().query(
				ChannelConstants.TABLE_NAME, ChannelConstants.ALL,
				ChannelConstants.SERVER + " = " + serverId, null, null, null,
				null);

		while (cursor.moveToNext()) {
			String channel = cursor.getString(cursor
					.getColumnIndex(ChannelConstants.NAME));
	 ...
	 * @param channel
	 */
	public void removeFavorite(String channel) {
		this.getWritableDatabase().delete(FavoriteConstants.TABLE_NAME,
				FavoriteConstants.NAME + " = ?", new String[] { channel });
	}

	public boolean addFavorite(String name) {
		// before adding, check if it's not already there
		Cursor cursor = this.getReadableDatabase().query(
				FavoriteConstants.TABLE_NAME, FavoriteConstants.ALL,
				FavoriteConstants.NAME + " = ?", new String[] { name }, null,
				null, null);
		if (cursor.getCount() == 0) {
			// not there
			cursor.close();
			ContentValues values = new ContentValues();
			values.put(FavoriteConstants.NAME, name);
			this.getWritableDatabase().insert(FavoriteConstants.TABLE_NAME,
					null, values);
			return true;
		} else {
			// already there
	...
	public ArrayList<String> getFavorites() {
		...
		Cursor cursor = this.getReadableDatabase().query(
				FavoriteConstants.TABLE_NAME, FavoriteConstants.ALL, null,
				null, null, null, FavoriteConstants.NAME + " ASC");
	...
	public void removeServerById(int serverId) {
		...
		if (identityId != -1) {
			...
			this.getWritableDatabase().execSQL(
					"DELETE FROM " + IdentityConstants.TABLE_NAME + " WHERE "
							+ IdentityConstants._ID + " = " + identityId + ";");
		}

		// Now delete the server entry
		this.getWritableDatabase().execSQL(
				"DELETE FROM " + ServerConstants.TABLE_NAME + " WHERE "
	...
	protected void setAliases(long identityId, List<String> aliases) {
		...
		for (String alias : aliases) {
			...
			getWritableDatabase().insert(AliasConstants.TABLE_NAME, null,
					values);
		}
	}
	...
	protected void deleteAliases(long identityId) {
		getWritableDatabase().execSQL(
				"DELETE FROM " + AliasConstants.TABLE_NAME + " WHERE "
	...
	protected List<String> getAliasesByIdentityId(long identityId) {
		...
		Cursor cursor = this.getReadableDatabase().query(
				AliasConstants.TABLE_NAME, AliasConstants.ALL,
				AliasConstants.IDENTITY + " = " + identityId, null, null, null,
				null);

		while (cursor.moveToNext()) {
			aliases.add(cursor.getString(cursor
					.getColumnIndex(AliasConstants.ALIAS)));
		}
		cursor.close();
	...
	public long addIdentity(String nickname, String ident, String realname,
		...
		long identityId = this.getWritableDatabase().insert(
				IdentityConstants.TABLE_NAME, null, values);
	 ...
	 * @param realname
	 */
	public void updateIdentity(int identityId, String nickname, String ident,
			String realname, List<String> aliases) {
		ContentValues values = new ContentValues();
		...
		this.getWritableDatabase().update(IdentityConstants.TABLE_NAME, values,
				IdentityConstants._ID + " = " + identityId, null);
	...
	public Identity getIdentityById(int identityId) {
		...
		Cursor cursor = this.getReadableDatabase().query(
				IdentityConstants.TABLE_NAME, IdentityConstants.ALL,
				IdentityConstants._ID + "=" + identityId, null, null, null,
				null);

		if (cursor.moveToNext()) {
			identity = new Identity();

			identity.setNickname(cursor.getString(cursor
					.getColumnIndex(IdentityConstants.NICKNAME)));
			identity.setIdent(cursor.getString(cursor
					.getColumnIndex(IdentityConstants.IDENT)));
			identity.setRealName(cursor.getString(cursor
					.getColumnIndex(IdentityConstants.REALNAME)));
	...
	public int getIdentityIdByServerId(int serverId) {
		...
		Cursor cursor = this.getReadableDatabase().query(
				ServerConstants.TABLE_NAME, ServerConstants.ALL,
				ServerConstants._ID + " = " + serverId, null, null, null, null);

		if (cursor.moveToNext()) {
			identityId = cursor.getInt(cursor
					.getColumnIndex(ServerConstants.IDENTITY));
		}
